      ...
      select case (Right)
       ...
       case (1)
        do k = 1,xNumPoints-1
         ...
         do l = 1,LegPoints
          x = xIntScale*xLeg(l)+xScaledZero
          u(l,k,Count) = MYBSpline(Order,Deriv,xNumPoints,xPoints,xNumPoints+Order-2,x)+MYBSpline(Order,Deriv,xNumPoints,xPoints,xNumPoints+Order-1,x)
         enddo
        enddo
       case(2)
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      double precision function  BasisPhi(x,Left,Right,Order,RDim,xPoints,xNumPoints,Deriv,Count)
      implicit none
...
!c      print*,'Count = ',Count,'Left = ',Left,'Right = ',Right
      ...
      case (1)
         if(Count.eq.1) then
            BasisPhi = MYBSpline(Order,Deriv,xNumPoints,xPoints,1,x) + MYBSpline(Order,Deriv,xNumPoints,xPoints,2,x)
         endif
         if(Count.gt.1) then
            BasisPhi = MYBSpline(Order,Deriv,xNumPoints,xPoints,Count+1,x)
         endif
      ...
      case (0)
         if(Count.eq.RDim) then
            BasisPhi = MYBSpline(Order,Deriv,xNumPoints,xPoints,xNumPoints+Order-2,x)
         endif
      case (1)
         if(Count.eq.RDim) then
            BasisPhi = MYBSpline(Order,Deriv,xNumPoints,xPoints,xNumPoints+Order-2,x)+MYBSpline(Order,Deriv,xNumPoints,xPoints,xNumPoints+Order-1,x)
         endif
      case (2)
         if(Count.eq.RDim) then
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      double precision function MYBSpline(Order,Deriv,xNumPoints,xPoints,n,x)
